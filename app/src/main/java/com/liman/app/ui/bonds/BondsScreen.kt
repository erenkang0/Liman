...
fun BondsScreen(
    ...
    LazyColumn(
        ...
        item {
            Text("Bağların", style = MaterialTheme.typography.displaySmall)
        }
        ...
        if (contacts.isEmpty()) {
            ...
            items(contacts, key = { it.id }) { contact ->
                ContactRow(contact, today) { onOpenContact(contact.id) }
            }
        }
    }
}
...
@Composable
private fun ContactRow(contact: Contact, today: LocalDate, onClick: () -> Unit) {
